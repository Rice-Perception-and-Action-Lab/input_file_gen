...
function createInputFile() {
	...
	if (sceneList) {
		for (i = 0; i < sceneList.length; i++) {
			var curScene = JSON.parse(sceneList[i]);
			var boolSound = (curScene.playSound === "true"); //required to parse the string value to boolean
            var trial = {
				trialNum: (i + 1),
				trialName: curScene.sceneName,
				corrAns: parseFloat(curScene.corrAns),
				playSound: boolSound,
				soundFile: curScene.soundFile,
	...
	//var filepath = path.join(appPath, filename, '..', '..');//i don't understand why this isn't working
